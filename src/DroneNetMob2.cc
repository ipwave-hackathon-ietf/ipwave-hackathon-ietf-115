...
//        std::cout << (par("constraintAreaMaxX")).doubleValue() <<"==============" <<std::endl;
...
//        targetPosition = missionPathNextDest_n(mBasePos.NodePos);
       ...
       if (std::strcmp(getParentModule()->getFullName(), "drone[0]") == 0){
            std::cout << " Name -----> " << getParentModule()->getFullName() << " DroneNetMob.cc" <<std::endl;
            Coord grLimits;
            grLimits.x = par("constraintAreaMaxX").doubleValue();
            grLimits.y = par("constraintAreaMaxY").doubleValue();
            grLimits.z = par("constraintAreaMaxZ").doubleValue();
