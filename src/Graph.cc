...
//hohyeon
Graph::Graph(int vertexNum, int edge_num, std::vector<int> start,std::vector<int> end, std::vector<double> weight){
    _import_from_data(vertexNum, edge_num, start,end, weight);

}
//hohyeon end
Graph::Graph( const Graph& graph )
{
    std::cout <<"Graph() called "<<std::endl;
...
//hohyeon
void Graph::set_edge_weight(int start_vertex, int end_vertex, double edge_weight){
	///3.2.1 construct the vertices
	BaseVertex* start_vertex_pt = get_vertex(start_vertex);
	BaseVertex* end_vertex_pt = get_vertex(end_vertex);
...
void Graph::_import_from_data(int vertexNum, int edge_num, std::vector<int> start,std::vector<int> end, std::vector<double> weight)
{
	clear();
...
/*
	if(m_nVertexNum != m_vtVertices.size())
	{
		std::cout << "The-number of nodes in the graph is "<<  m_vtVertices.size() << " instead of " << m_nVertexNum << std::endl;
		//exit(1);
	}
*/
	m_nVertexNum = m_vtVertices.size();
	m_nEdgeNum = m_mpEdgeCodeWeight.size();
//	std::cout << "The number of nodes in the graph is "<<  m_nVertexNum << " and edge# is " << m_nEdgeNum << std::endl;
}
//hohyeon end
